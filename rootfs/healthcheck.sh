...
LASTLOG_PACKETS_SENT=$(grep "Packets sent in the last 60 seconds" "$SXFEEDER_LOG_FILE" | tail -1 | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g")
...
# TODO:
# Entries in /var/log/sxfeeder/current to search for
